...
def generate_ai_content(d):
    ...
    # Pre-compute all conditional strings to avoid f-string nesting errors
    ...
    gld_txt  = "is acting as a safe haven" if gld[1]>0 else "pulled back on profit-taking"
    ...
    rut_rel  = "outperforming" if d.get("^RUT",(0,0))[1] > sp[1] else "underperforming"
    rut_txt  = "value rotation signal worth monitoring." if d.get("^RUT",(0,0))[1] > sp[1] else "risk-off confirmed across size spectrum."
    t30v     = d.get("^TYX",(0,0))[0]
    fed_txt  = "pricing in at least one rate hike this year" if t10>4.6 else "pricing the Fed on hold through year-end"
    stag_note= "above 4.75%" if t10<4.75 else "above 5.0%"

    results["macro"] = (
        "<b>" + sp_dir + " — " + TODAY + "</b><br><br>"
        "Markets " + sp_verb + " today with the S&P 500 "
        + sp_move + " " + f"{abs(sp[1]):.2f}%" + " to " + f"{sp[0]:,.2f}. "
        "The Nasdaq " + nq_rel + f" at {nq[1]:+.2f}%. "
        ...
        "keeping inflation elevated. The 10Y Treasury at " + f"{t10:.2f}%" + " "
        + rate_txt + ". "
        f"Gold at ${gld[0]:,.0f} " + gld_txt + f". Bitcoin at ${btc[0]:,.0f}.<br><br>"
        ...
        "Peace talks in the Middle East remain the wildcard — any ceasefire would send oil lower and give the Fed room to cut."
    )
    ...
    results["rates"] = (
        "<b>Rates — " + f"{t10:.2f}%" + " on the 10Y as Fed stays on hold</b><br><br>"
        "Treasury yields today with the 10Y at "
        "<span class=\"" + ("danger" if t10>4.6 else "warn") + "\">" + f"{t10:.2f}%" + "</span> "
        f"and the 30Y at {t30v:.2f}%. "
        ...
        "on discounted cash flow models. REITs, utilities, and long-duration tech are most exposed. "
        "Short-duration value, energy, and financials are the relative winners in this environment."
    )
